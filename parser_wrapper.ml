...
let conv_primitive_app t ts typ =
  ...
  | Var {Id.name="Pervasives.raise"}, [t] -> {desc=Raise(t); typ=typ; attr=ANone}
  | Var {Id.name="Pervasives.ref"}, [t] -> make_ref t
  | Var {Id.name="Pervasives.read_int"}, [{desc=Const Unit}] -> make_app {randint_term with attr = AAbst_under} [unit_term]
...
let rec from_pattern {Typedtree.pat_desc=desc; pat_loc=_; pat_type=typ; pat_env=env} =
  ...
  let desc =
    ...
    | Tpat_constant(Const_int n) -> PConst {desc=Const(Int n);typ=typ'; attr=ANone}
    | Tpat_constant(Const_char c) -> PConst {desc=Const(Char c);typ=typ'; attr=ANone}
    | Tpat_constant(Const_string s) -> PConst {desc=Const(String s);typ=typ'; attr=ANone}
    | Tpat_constant(Const_float s) -> PConst {desc=Const(Float s);typ=typ'; attr=ANone}
    | Tpat_constant(Const_int32 n) -> PConst {desc=Const(Int32 n);typ=typ'; attr=ANone}
    | Tpat_constant(Const_int64 n) -> PConst {desc=Const(Int64 n);typ=typ'; attr=ANone}
    | Tpat_constant(Const_nativeint n) -> PConst {desc=Const(Nativeint n);typ=typ'; attr=ANone}
...
let rec from_expression {exp_desc=exp_desc; exp_loc=_; exp_type=typ; exp_env=env} =
  ...
  | Texp_constant c ->
      {desc = Const (from_constant c); typ = typ'; attr=ANone}
  ...
  | Texp_function(_,pats,totality) ->
      ...
      in
      make_fun x {desc=Match(make_var x, List.map aux pats@tail);typ=typ2; attr=ANone}
  | Texp_apply(e, es) ->
      let t = from_expression e in
      let aux = function
  ...
  | Texp_match(e,pats,tp) ->
      ...
      in
      {desc=Match(t, pats''); typ=typ'; attr=ANone}
  | Texp_try(e,pats) ->
      let aux (p,e) =
        match e.exp_desc with
        | Texp_when(e1,e2) -> from_pattern p, from_expression e1, from_expression e2
      ...
      let pats' = List.map aux pats in
      let pats'' = pats' @ [make_pany !typ_excep, true_term, {desc=Raise(make_var x); typ=typ'; attr=ANone}] in
      {desc=TryWith(from_expression e, make_fun x {desc=Match(make_var x, pats''); typ=typ'; attr=ANone}); typ=typ'; attr=ANone}
  | Texp_tuple es ->
      {desc=Tuple(List.map from_expression es); typ=typ'; attr=ANone}
  ...
  | Texp_record(fields,None) ->
      ...
      {desc=Record fields''; typ=typ'; attr=ANone}
        ...
        let aux lbl =
          ...
          with Not_found ->
            name, (flag, {desc=Field(lbl.lbl_pos, name, flag,
                                    {desc=Var r; typ=Id.typ r; attr=ANone});
                          typ=from_type_expr env lbl.lbl_arg;
                          attr=ANone})
        in
        List.map aux labels
      in
      make_let [r, [], from_expression init] {desc=Record fields';typ=typ'; attr=ANone}
  ...
  | Texp_ifthenelse(e1,e2,e3) ->
      ...
      let t3 =
        match e3 with
          None -> {desc=Const Unit; typ=TUnit; attr=ANone}
...
let from_use_file ast =
  ...
  let aux t = function
    ...
    | Decl_exc _ -> t
  in
  let t = List.fold_left aux {desc=Const Unit;typ=TUnit; attr=ANone} defs in
