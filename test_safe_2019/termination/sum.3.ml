let rec sum (prev_set_flag_sum_35:bool) (s_prev_sum_n_34:int) (n:int) =
  if prev_set_flag_sum_35
  then
    if s_prev_sum_n_34 > n && n >= 0 then () else assert false;
...
and sum_without_checking_43 (_:bool) (_:int) (n:int) =
  ...
  then
    ...
    n + sum_without_checking_43 set_flag_sum_36 s_sum_n_33 (n - 1)
and u_31 = sum false 0 (Random.int 0)
let u_1114 = ()
