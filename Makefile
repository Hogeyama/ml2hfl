...
CMO = environment.cmo flag.cmo util.cmo color.cmo tree.cmo		\
	...
	type_decl.cmo term_util.cmo spec.cmo spec_parser.cmo		\
	spec_lexer.cmo CEGAR_type.cmo CEGAR_syntax.cmo			\
	CEGAR_print.cmo typing.cmo ref_type.cmo type_check.cmo		\
	trans.cmo lift.cmo CEGAR_ref_type.cmo CEGAR_util.cmo		\
	CEGAR_lift.cmo slicer.cmo useless_elim.cmo inter_type.cmo	\
	type_trans.cmo fpatInterface.cmo CPS.cmo curry.cmo		\
	CEGAR_CPS.cmo parser_wrapper.cmo encode_list.cmo		\
	encode_rec.cmo CEGAR_abst_util.cmo CEGAR_trans.cmo		\
	CEGAR_abst_CPS.cmo CEGAR_abst.cmo CEGAR_parser.cmo		\
	CEGAR_lexer.cmo trecs_parser.cmo trecs_lexer.cmo		\
	trecs_syntax.cmo trecsInterface.cmo ModelCheck_util.cmo		\
...
bin: $(NAME).opt
	...
	@cp $(CVC3) $(TRECS) $(NAME).opt $(MOCHI_BIN_DIR)/bin
