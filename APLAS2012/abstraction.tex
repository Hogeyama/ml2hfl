...
by using the abstraction type\footnote{\cc{Abstraction types are types to
express which predicate should be used to abstract each value.
See our previous
...
If $E$ is empty, the abstract relation is the same as the
...
\infrule[A-AppExp]
 {E(x) = \Abs{x_1}{\cdots\Abs{x_n}{e}} \andalso
  \Abst{\Gamma}{E}{e[v_1/x_1,\dots,v_{i-1}/x_{i-1}]}{\sigma}{e'}}
...
Here, an abstraction type of the form $\INT[P_1,\dots,P_n]$ means that a
value $v$ of that type should be abstracted to a tuple of
$(b_1,\dots,d_n)$, where $b_i$ denotes whether $P_i{v}$ holds or not.
