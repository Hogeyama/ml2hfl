...
that is abstracted from functions that is not abstracted.  For example,
the following program can verify by abstracting only \texttt{sum} and
\texttt{main} as stated in Sect.~\ref{sec:intro}.
...
abstraction is defined as the relation $\AbstPLDI{\Gamma}{t}{\tau}{t'}$
that reads $t$ is abstracted to $t'$ by using the abstraction
type\footnote{Abstraction types are types to express which predicate
should be used for each variable.  We do not show the definition of
abstraction types.  See our previous paper~\cite{KobayashiPLDI2011} for
details.}  $\tau$ under the assumption that each free variable x of $t$
has been abstracted using the abstraction type $\Gamma(x)$.  We extend
predicate abstract relation by adding extracting function definitions
$E$ like $\Abst{\Gamma}{E}{t}{\tau}{t'}$.  Here, functions in $E$ do not
abstracted and are extracted in abstraction.  For example, the
abstraction of the program above is represented as the relation
$\Abst{\mathtt{sum}\COL\tau_\mathtt{sum},\mathtt{main}\COL\tau_\mathtt{main}}{\mathtt{add}=\Abs{x}{\Abs{y}{x+y}}}{t}{\tau}{t'}$.
...
Figure~\ref{fig:abstraction} defines selective predicate abstraction
relation.  Figure~\ref{fig:abstraction} shows only the rules for
applications and let-expressions.  Other rules are same as the previous
predicate abstraction relation~\cite{KobayashiPLDI2011} except that the
extracting function definition $E$ is added.  The relation
...
replaced with the abstraction of $E(x)$.
$E=\set{f_{i_1}=v_{i_1},\dots,f_{i_k}=v_{i_k}}$ is a subset of a
program.  A function definition is in E intuitively means the function
is expanded in the abstraction. If $E$ is the empty set, the abstract
...
For some $D$ and $E$, there is no $D'$ such that
$\AbstProg{\Gamma}{E}{D}{D'}$ when there is no cyclic definitions in $E$
like $\set{f = \Abs{x}{\App{f}{x}}}$.  The theorem below shows the
algorithm to select which functions should be in $E$.

\begin{theorem}
 The function call relation $f_i \call_E f_j$ is defined as $f_j \in
 \FV{E(f_i)}$.  If $\AbstProg{\Gamma}{\emptyset}{D}{D'}$, and $f_i
 \call_E^+ f_i$ does not hold for each function $f_i \in \Dom{E}$, then
 there exists $D''$ such that $\AbstProg{\Gamma}{E}{D}{D''}$.
\end{theorem}

We discuss properties of selective predicate abstraction. We show that
