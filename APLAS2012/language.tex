...
The source language is the call-by-value PCF with the syntax shown in
Fig.~\ref{fig:source-syntax}.
%  A program consists a set of top-level functions.
The meta-variable $\OP$ ranges over the sets of operators over integers.
The expressions are standard except that $\FAIL$ aborts a program.  We
assume that every function in $D$ has a function type and $D$ contains a
distinguished function symbol $\mathit{main} \in \set{f_1,\dots,f_n}$
whose type is $\TFun{\INT}{\INT}$.
...
The goal of our verification is to check whether $\App{\mathit{main}}{0}
\reds_D \FAIL$.  We can reduce the verification of the
absence of assertion failures and pattern matching failures to
verification of the property $t \reds_D \FAIL$ by using a trivial
transformation. For example, a program ``{\tt match t with ...}'' cause
a pattern match failure if and only if a program ``{\tt match t with
... | {\us} -> $\FAIL$}'' execute $\FAIL$.
