...
We discuss each problems and explain our approach below.
...
\item The main bottleneck of the previous version of MoCHi was the
      ...
      $\Abs{y}{y\geq0}$ for the second argument of \texttt{add} and
...
\begin{alltt}
let add u by = if by then true else rand_bool()
let rec sum _ = if rand_bool() then true
                else add (if sum () then true else *)
let main x = assert (sum ())
\end{alltt}
      Here, \texttt{rand\us{}bool()} means a non-deterministic boolean value.
      \texttt{by} denotes that $\mathtt{x} \geq 0$.
      If we abstract the program using only the predicate
      $\Abs{r}{r\geq x}$ for return values of \texttt{sum}, we get the
...
\end{alltt}
      ...
      abstraction is too coarse.  Automatically finding all such appropriate
      predicates is inefficient.  In the previous
      paper~\cite{KobayashiPLDI2011}, we have adapted CEGAR to find such
      predicates, but the technique is necessarily heuristic, and often
      fails.
...
\end{alltt}
%Function \texttt{check} may cause an assertion failure when two arguments are given.
This program is translated into the following order-5
...
formalize this transformation, called \emph{selective CPS transformation}.
