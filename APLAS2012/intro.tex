...
\end{alltt}
      ...
      predicate $\Abs{r}{r \geq x}$ for return values of \texttt{sum}
...
\item Another efficiency problem is caused by CPS transformation.  As
      ...
      gap between evaluation strategies of higher-order recursion schemes and
...
\item The third problem is the insufficiency of supported language
      ...
      To support recursive data structures without an interpolating
      theorem prover for these, we translate programs with recursive
      data structures into ones with integers.  The transformation is
      carried out by encoding recursive data structures and control
      operations using higher-order functions.  For example, a list is
