...
let rec associate_of x = function
  | [] -> None
  | (key, value) :: l ->
      if x = key then Some value else associate_of x l;;

let rec associate_in_list key = function
  | [] -> None
  | (list_of_keys, value) :: rest ->
      if member key list_of_keys then Some value
...
let is_goodbye ph = Random.bool ()(*(ph = ["so"; "long"]) || (ph = ["bye"]) ||
...
let answer_patient () =
  ...
  let possible_answers =
      match associate_of (synonym_of_phrase phrase)
                     answers_to_simple_phrases
      with Some value -> value
          | None ->
match
       associate_of_an_element_of phrase
      ...
      then answers_to_questions
        ...
        match associate_of_an_element_of phrase
