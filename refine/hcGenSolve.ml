...
let rec solve ch hcs0 =
  ...
  *)
  ...
  let _ =
    if false then
      let npid = List.length pids in
      Format.printf "#pids: %d@," npid
  in
  ...
  let sol =
    if pidss = [] then
      ...
      let sol, hcs =
        ...
        let pids, lb_ub_of =
          let pids, lbs, ubs =
            let _ = Global.log (fun () -> Format.printf "Horn clauses:@,  %a@," HornClause.pr hcs0) in
            let lbs = compute_lbs hcs0 in
            let _ = Global.log (fun () -> Format.printf "lower bounds:@,  %a@," TypPredSubst.pr lbs) in
            if false then (** upper bounds computation is too slow *)
              let ubs, ndpids = compute_ubs hcs0 in
              ...
              try
                let pids, ubs =
                  Util.find_app
                    (fun pids ->
                      let ubs, ndpids = compute_ubs_pids pids hcs0 in
                      let pids = Util.diff pids ndpids in
                      if pids = [] then
                        None
                      else
          ...
          let lb_ub_of =
            fun pid ->
              try
                ...
                let atm = Atom.of_pred (pid, xtys) in
                xtys,
                TypPredSubst.lookup_map atm lbs,
                TypPredSubst.lookup_map atm ubs
              with Not_found ->
        ...
        let pidss =
          ...
          in
          let pidss = Util.sort_by_dec List.length (classify pids []) in
          let _ =
            Global.log (fun () ->
        ...
        let sol, hcs =
          if List.length pids = 1 then
            sol, List.map (TypPredSubst.subst sol) hcs0
          else
            if true then
              let sol0 =
