...
let solve_aux ch hcs0 pidss hcs ndpids lbs ubs =
  let _ = Global.log_begin "HcGenSolve.solve_aux" in
  let pids =
    (* pick one from pidss *)
    try
      Util.find_app
  ...
  let sol, hcs =
    if List.length pids = 1 then
      ...
      if true then
        ...
        valid_sol sol0 hcs0
  in
  let _ = Global.log_end "HcGenSolve.solve_aux" in
  sol, hcs
  ...
  let sol =
    if pidss = [] then
      ...
      let ndpids, lbs, ubs =
        ...
        let ubs, ndpids = compute_ubs hcs in
        let _ = Global.log (fun () -> Format.printf "upper bounds:@,  %a@," TypPredSubst.pr ubs) in
        ndpids, lbs, ubs
      in
      let sol, hcs = solve_aux ch hcs0 pidss hcs ndpids lbs ubs in
      sol @ solve ch hcs
  in
