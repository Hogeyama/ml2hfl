...
let rec subst sub aty =
  match aty with
    Base(bty, x, ts) ->
      ...
      Base(bty, x, List.map (FormulaUtil.subst sub') ts)
  | Fun(aty1, aty2) ->
      Fun
        (subst sub aty1,
...
let rec merge2 aty1 aty2 =
  match aty1, aty2 with
    Base(bty1, x1, ts1), Base(bty2, x2, ts2) ->
      ...
      Base(bty1, x, List.unique ((List.map (FormulaUtil.subst sub1) ts1) @ (List.map (FormulaUtil.subst sub2) ts2)))
...
let of_interaction_type f sty =
  ...
  let subst = FormulaUtil.subst (fun x -> Term.make_var (List.assoc x env)) in
...
let rec of_refinement_type rty =
  match rty with
    RefType.Base(x, RefType.Unit, t) ->
      let ps = if !Global.extract_atomic_predicates then Formula.atoms t else if t = Formula.ttrue || t = Formula.tfalse then [] else [t] in
      Base(Unit, x, ps)
