...
let simplify (pid, ttys) =
  pid, List.map (fun (t, ty) -> LinArith.simplify t, ty) ttys

let subst sub (pid, ttys) =
  pid, List.map (fun (t, ty) -> FormulaUtil.subst sub t, ty) ttys

let subst_fixed sub (pid, ttys) =
  pid, List.map (fun (t, ty) -> FormulaUtil.subst_fixed sub t, ty) ttys

let equiv env (pid1, ttys1) (pid2, ttys2) =
  pid1 = pid2 &&
  Cvc3Interface.implies
    env
    (List.map2 (FormulaUtil.simplify -|| Formula.eq_tty) ttys1 ttys2)
  ...
  List.for_all2
    (fun tty1 tty2 ->
      ...
      (List.for_all (fun x -> not (p x)) (TypTerm.fvs tty1) (* @todo *) &&
       Cvc3Interface.implies
         env
         [FormulaUtil.simplify (Formula.eq_tty tty1 tty2)]))
    ttys1 ttys2
