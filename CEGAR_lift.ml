...
let rec lift_term xs = function
  ...
  | Let(f,t1,t2) ->
      ...
      let f'' = make_app (Var f') (List.map _Var xs) in
      let t1'' = List.fold_left2 (fun t x x' -> subst_var x x' t) t1' ys ys' in
      let xs' = xs@ys' in
      let defs1,t1''' = lift_term xs' t1'' in
  ...
  | Fun _ as t ->
      ...
      let f = new_id "f" in
      let ys' = List.map (fun x -> if List.mem x xs then rename_id x else x) ys in
      let t'' = List.fold_left2 (fun t x x' -> subst_var x x' t) t' ys ys' in
      let xs' = xs@ys' in
      let f' = make_app (Var f) (List.map _Var xs) in
...
let rec lift_term2 xs = function
  ...
  | Let(f,t1,t2) ->
      let ys,t1' = decomp_fun t1 in
      let fv = List.Set.inter xs (List.Set.diff (get_fv t1) ys) in
      ...
      let f' = rename_id f in
      let f'' = make_app (Var f') (List.map _Var fv) in
...
let lift2 {defs; main; attr} =
  let defs = List.flatten_map lift_def2 defs in
  let prog =  {env=[]; defs; main; attr} in
