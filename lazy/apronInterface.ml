...
let linconstr_of env t =
  let c, nxs, n = Term.int_rel_of t in  
  let expr = Apron.Linexpr1.make env in
  Apron.Linexpr1.set_array expr
    (Array.of_list (List.map (fun (n, x) -> Apron.Coeff.s_of_int n, Apron.Var.of_string (Var.string_of x)) nxs))
    (Some ((Apron.Coeff.s_of_int n)));
  let mexpr = Apron.Linexpr1.make env in
  Apron.Linexpr1.set_array mexpr
    (Array.of_list (List.map (fun (n, x) -> Apron.Coeff.s_of_int n, Apron.Var.of_string (Var.string_of x)) (Arith.minus nxs)))
    (Some ((Apron.Coeff.s_of_int n)));
		match c with
    ...
    (fun t (n, x) ->
      if n = 0 then
        t
		      ...
		      let t' = if n = 1 then Term.make_var2 x else Term.mul (Term.make_int n) (Term.make_var2 x) in
				    match t with
