...
let test_sum_assert () =
  ...
  let sum1 = { Fdef.attr = []; Fdef.name = Idnt.make "sum"; Fdef.args = [Idnt.make "x"]; Fdef.guard = Term.leq (Term.make_var "x") (Term.tint 0); Fdef.body = Term.tint 0 } in
  let sum2 = { Fdef.attr = []; Fdef.name = Idnt.make "sum"; Fdef.args = [Idnt.make "x"]; Fdef.guard = Term.gt (Term.make_var "x") (Term.tint 0); Fdef.body = Term.add (Term.make_var "x") (Term.apply (Term.make_var "sum") [Term.sub (Term.make_var "x") (Term.tint 1)])} in
  let assert1 = { Fdef.attr = []; Fdef.name = Idnt.make "assert"; Fdef.args = [Idnt.make "b"]; guard = Term.eq (Term.make_var "b") (Term.ttrue); Fdef.body = Term.tunit} in
  let assert2 = { Fdef.attr = []; Fdef.name = Idnt.make "assert"; Fdef.args = [Idnt.make "b"]; guard = Term.eq (Term.make_var "b") (Term.tfalse); Fdef.body = Term.tevent "fail"} in
  let tymain = SimType.Fun(SimType.Int, SimType.Unit) in
  let tysum = SimType.Fun(SimType.Int, SimType.Int) in
...
let test_copy_copy () =
  ...
  let copy1 = { Fdef.attr = []; Fdef.name = Idnt.make "copy"; Fdef.args = [Idnt.make "x"]; Fdef.guard = Term.eq (Term.make_var "x") (Term.tint 0); Fdef.body = Term.tint 0 } in
  let copy2 = { Fdef.attr = []; Fdef.name = Idnt.make "copy"; Fdef.args = [Idnt.make "x"]; Fdef.guard = Term.neq (Term.make_var "x") (Term.tint 0); Fdef.body = Term.add (Term.tint 1) (Term.apply (Term.make_var "copy") [Term.sub (Term.make_var "x") (Term.tint 1)])} in
  let check1 = { Fdef.attr = []; Fdef.name = Idnt.make "check"; Fdef.args = [Idnt.make "x1"; Idnt.make "x2"]; guard = Term.eq (Term.make_var "x1") (Term.make_var "x2"); Fdef.body = Term.tunit} in
  let check2 = { Fdef.attr = []; Fdef.name = Idnt.make "check"; Fdef.args = [Idnt.make "x1"; Idnt.make "x2"]; guard = Term.neq (Term.make_var "x1") (Term.make_var "x2"); Fdef.body = Term.tevent "fail"} in
  let tymain = SimType.Fun(SimType.Int, SimType.Unit) in
