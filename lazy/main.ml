...
let test1 () =
  ...
  let check1 = { Fdef.attr = []; Fdef.name = Id.make "check"; Fdef.args = [Id.make "x1"; Id.make "x2"]; guard = Term.geq (Term.make_var "x1") (Term.make_var "x2"); Fdef.body = Term.make_event "end"} in
  ...
  let rt = Ctree.Node(Term.apply (Term.Var([], prog.Prog.main)) (List.map (fun arg -> Term.Var([], arg)) (Prog.get_args prog.Prog.main.Id.id prog)), ref []) in
  let rec loop wl =
