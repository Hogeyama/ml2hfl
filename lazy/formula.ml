...
(** ensure: the result does not use () if t does not use any unit variable and
    t1 =b t2 and t1 <>b t2 are resp. replaced with t1 iff t2 and not (t1 iff t2).
...
let rec simplify t =
  ...
  | Const(attr, c), [t1; t2] when Const.is_ibin c ->
      let t1, t2 = terms_of_arith (arith_of (sub t1 t2)) in
...
*)
  ...
  | Const(attr, c), [t1; t2] when c = Const.EqUnit ->
      let t1 = simplify t1 in
      let t2 = simplify t2 in
      if t1 = t2 then
        ttrue
        ...
        apply (Const(attr, c)) [t1; t2]
  | Const(attr, c), [t1; t2] when c= Const.NeqUnit ->
      let t1 = simplify t1 in
      let t2 = simplify t2 in
      if t1 = t2 then
        tfalse
