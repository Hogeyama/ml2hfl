              ...
              Util.partition_map
                (fun (x_uid, trs) ->
                  if List.exists (fun (x_uid') -> dl_ancestor_of x_uid' x_uid) callee then
                    let _ = let (x, uid) = x_uid in assert true(*the length of the path of %a is odd*) in
                    `L(trs)
                  else
                    `R(x_uid, trs))
                sub
    ...
    try
      ...
      let p op (x, uid) trs =
        ...
        (match List.filter (function Cnode(_, _, _) -> true | _ -> false) trs with
          [] -> true
        | [Cnode(_, (y, _), _)] -> not (Var.is_pos y)
        | _ -> false))
      in
      let ctx, tr = call_of p eptr in
      let sums', eptr =
        try
          summary_of ctx tr [](*(rec_callers_of eptr tr)*)
        with CsisatInterface.No_interpolant ->
