...
let rec print_list_aux newline print punc last fm xs =
  match xs with
      [] -> ()
    | [x] when last ->
    ...
    | x::xs ->
        Format.fprintf fm "%a" print x;
        Format.fprintf fm punc;
        if newline then Format.fprintf fm "@,";
        Format.fprintf fm "%a" (print_list_aux newline print punc last) xs
let print_list print ?(newline=false) ?(first=false) ?(last=false) punc fm xs =
