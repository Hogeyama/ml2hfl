...
predicate discovery which uses only a single infeasible error path
instead of multiple infeasible error paths, unlike the method described
in Section~\ref{sec:refine}.  While consideration of multiple paths
enhances the precision of predicate abstraction, it slows down
verifications of some programs (such as ``map\_filter'') since the
implementation is preliminary.  There are programs which can be verified
when using multiple paths, and cannot be verified when using only a single
path.  The following is such a program.
\begin{alltt}
...
\end{alltt}
The program can be successfully verified if we use a predicate
$\LENGTH(\mathtt{acc}) + \LENGTH(\mathtt{xs}) = \LENGTH(\mathtt{r})$ on
the argument $\mathtt{acc}$, $\mathtt{xs}$ and the return value
$\mathtt{r}$ of \texttt{length}.  MoCHi discovered the predicate and
verified the program in 0.36 seconds with 3 CEGAR-cycles when using
multiple paths.
