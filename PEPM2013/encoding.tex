...
We treat only algebraic data types, i.e., the finite combination of sum
types and product types.  In an algebraic data type, we require that
recursive type variables cannot occur under function constructors. Thus,
$\RecType{\alpha}{\UNIT+(\TFun{\INT}{\INT})*\alpha}$ (which corresponds
to $\List{(\TFun{\INT}{\INT})}$) is OK, but neither
