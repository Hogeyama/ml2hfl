...
The extended algorithm proceeds as follows:
\begin{enumerate}
...
\item Substitute \(\lambda \seq{x}.\phi\) for some predicate variables 
in \(S\) that occur in \(C\) to get \(C'\) and repeat the entire 
procedure if \(C'\) still contains a predicate variable.
\end{enumerate}
...
\paragraph{Finding a candidate solution \(\lambda \seq{x}.\phi\):}
...
\paragraph{Substituting the candidate solution \(\lambda \seq{x}.\phi\) to \(C\):}
Note that we cannot always substitute all the predicates 
...
substitute the candidate solution \(\lambda \nu.0 \leq \nu\leq 1\) for 
either \(Q_1\) or \(Q_2\).

Therefore, we find a maximal nonempty subset \(\{R_1,\dots,R_m\}\) of 
\(S\) for which we can safely substitute \(\lambda\seq{x}.\phi\) (i.e., 
the substituted set of constraints 
