...
let rec same typ1 typ2 =
  match typ1,typ2 with
    ...
    | TVar _, TVar _ -> true
    | TFun(x1,typ1),TFun(x2,typ2) -> same (Id.typ x1) (Id.typ x2) && same typ1 typ2
    | TList(typ1,_), TList(typ2,_) -> same typ1 typ2
    | TConstr(s1,_),TConstr(s2,_) -> s1 = s2
    | TVariant stypss1,TVariant stypss2 ->
        let aux (s1,typs1) (s2,typs2) = s1=s2 && List.for_all2 same typs1 typs2 in
          List.length stypss1 = List.length stypss2 && List.for_all2 aux stypss1 stypss2
    | TRecord(_,fields1),TRecord(_,fields2) -> List.for_all2 (fun (s1,(_,typ1)) (s2,(_,typ2)) -> s1=s2 && same typ1 typ2) fields1 fields2
...
let rec print print_pred fm typ =
  ...
  let print_preds = print_preds print_pred in
    match typ with
      ...
      | TFun(x, typ) ->
          assert (not Flag.check_fun_arg_typ || same (Id.typ x) typ);
      ...
      | TList(typ,ps) ->
          if ps = []
          then Format.fprintf fm "%a list" print typ
          else Format.fprintf fm "%a list[%a]" print typ print_preds ps
      | TVariant ctypss ->
