...
type transition = ((state list * alpha) * state) list 
...
type automaton = {states: state list;  (* the set of states *)
                  ...
                  delta: transition ref;
                         (* (non-deterministic) transition function *)
                  gen: (state * state) list (* generalize relation *);
                  sub: (state * state) list ref (* subtype relation *);
                  cache_aty: (aty * state) list;
                  cache_ty: (ty * state) list
                 }


let empty_automaton = 
  {states=[0]; univ= ref [0]; delta=ref []; gen=[]; sub = ref []; cache_aty=[]; cache_ty=[([],0)]}

let union_delta delta1 delta2 = delta1@delta2
...
let rec find_eqrel (delta: transition): state list list * transition =
  match delta with
    [] -> ([], delta)
  | tr::delta' ->
      let (delta1, delta2) = 
          list_filter2 (same_dom tr) delta' in
      let states = delete_duplication_unsorted (List.map (fun (_,q) -> q) (tr::delta1)) in
      let (eqrel3, delta3) = find_eqrel delta2 in
        if List.length states >1 then
             let eqrel = merge_eqrel [states] eqrel3 in
                (eqrel, tr::delta3)
        else
          (eqrel3, tr::delta3)
...
let rec merge_states delta eqrel =
  let delta' = List.map (fun ((qs,a), q) -> 
                      ((List.map (replace eqrel) qs, a), replace eqrel q)) delta in
      delta'

let update_automaton_delta automaton delta =
  {states=automaton.states;
   univ=automaton.univ;
   delta=ref delta;
   gen=automaton.gen;
   sub=automaton.sub;
   cache_aty = automaton.cache_aty;
   cache_ty = automaton.cache_ty
  }
...
let minimize_automaton automaton =
   ...
   let delta' = delete_duplication_unsorted (merge_states !(automaton.delta) eqrel) in
   ...
   let univ' = List.filter 
      ...
      ({states = states'; univ=ref univ'; 
       delta=ref delta'; gen = automaton.gen; sub = automaton.sub;
        cache_ty = automaton.cache_ty; cache_aty = automaton.cache_aty}, eqrel)

(** merge_states :
  transition -> state list list -> transition * state list list 
 **)
(**
  let (new_eqrel, delta'') = find_eqrel delta' 
  in  
     if new_eqrel=[] then
        (delta'', eqrel)
     else
        merge_states delta'' (merge_eqrel eqrel new_eqrel)
**)

(*** obsolete
  let new_st = x@y in
  let states' = new_st::
                (del_states [x;y] automaton.states) in
  let delta' =
    List.map (fun ((z,a),next) ->
                ((replace [x;y] new_st z, a), List.map (replace [x;y] new_st) next))
     automaton.delta
 in
  let sub' = (x,new_st)::(y,new_st)::automaton.sub in
     {states=states'; delta = delta'; gen=sub'}
***)
...
let automaton1 =
  let q0 = gen_newstate() in 
  let q1 = gen_newstate() in 
  let q2 = gen_newstate() in 
  let q3 = gen_newstate() in 
  {states = [q0;q1;q2;q3]; univ = ref [];
   delta = ref [(([], Tbase("q0")), q0); (([q0], Tpair), q1); (([q1], Tpair), q2); (([q2], Tpair), q3)];
   gen = [];
   sub = ref [];
   cache_ty = []; cache_aty = []}
...
let automaton2 =
  ...
  let q3 = gen_newstate() in 
  {states = [q0;q1;q2;q3]; univ = ref [];
   delta = ref [(([], Tbase("q0")), q0); (([q0], Tpair), q1); (([q0], Tpair), q2); (([q2], Tpair), q3)];
   gen = [];
   sub = ref [];
   cache_ty = []; cache_aty = []}
...
let automaton3 =
  ...
  let q2 = gen_newstate() in 
  let q3 = gen_newstate() in 
  let q4 = gen_newstate() in 
  let q5 = gen_newstate() in 
  {states = [q0;q1;q2;q3;q4;q5]; univ = ref [];
   delta = ref [(([], Tbase("q0")), q0); (([q0], Tpair), q1); (([q1], Tpair), q4); (([q2], Tpair), q5); (([q1], Tpair), q2); (([q2], Tpair), q3)];
   gen = [];
   sub = ref [];
   cache_ty = []; cache_aty = []}
...
let rename_delta ren delta =
  List.map (fun ((qs,a),q) -> ((rename_states ren qs, a), rename_state ren q)) delta
(**
  match delta with
    [] -> (delta, ren)
  | ((qs,a),q)::delta' ->
        let (delta'', ren') = rename_states delta' ren old in
        let qs' = List.map (apply_ren ren') qs in 
        let q' = apply_ren ren' q in
          if (List.exists (new_st old) qs')&& (List.mem q' old)
          then 
             let q0 = gen_newstate() in
             let ren'' = (q,q0)::ren' in
               rename_states delta ren'' old
          else
             (((qs',a),q')::delta'', ren')
**)
...
(* return the set of states {q | q->* q' in staets} *)
let rec get_reachable_states delta states =
     let (delta1, delta2) = 
       list_filter2 (fun ((qs,a),q) -> 
                    non_disjoint qs states) delta in
     let states1 = delete_duplication_unsorted (List.map snd delta1) in
     let states' = union_st states states1 in
       if List.length states=List.length states' then
          states
       else
          get_reachable_states delta2 states'


(* generalize "automaton" by merging the states in "states" *)
(* gen_automaton : automaton -> state list -> automaton *)
let gen_automaton automaton states =
  if List.exists (is_univstate automaton) states
  then
    raise (Fatal "gen_automaton: universal states cannot be generalied")
  else
    let delta = merge_states !(automaton.delta) [states] in
    let automaton' = update_automaton_delta automaton delta in
    let (automaton'', eqrel) = minimize_automaton automaton' in
    let eqrel' = merge_eqrel states eqrel in 
    let new_states = get_reachable_states !(automaton''.delta) [List.hd states] in
    let delta1 = List.filter (fun (_,q)->List.mem q new_states) !(automaton''.delta) in
    let eqrel1 = List.map (find_eqclass eqrel') new_states in
    let ren = List.fold_left
              (fun r -> fun qs ->
                 let q = gen_newstate() in (List.map (fun q'->(q',q)) qs)@r)
              [] eqrel1 in
    let delta' = List.sort compare (rename_delta ren delta1) in
    let univ1 = rename_states ren !(automaton.univ) in
     {states = automaton.states@(List.map snd ren);
      univ = ref (union_st !(automaton.univ) univ1);
      delta = ref (union_delta !(automaton.delta) delta'); 
        (* TO BE CHECKED: this may not remove duplicated transition rules as delta may not be sorted *)
      gen = merge_and_unify compare automaton.gen ren;
      sub = automaton.sub;
      cache_aty = automaton.cache_aty;
      cache_ty = automaton.cache_ty}
...
let rec aty2automaton aty automaton =
  try
    let state = List.assoc aty automaton.cache_aty in
       (state, automaton)
  with Not_found ->
  let x = gen_newstate() in
  let cache_aty' = (aty,x)::automaton.cache_aty in
  let states' = x::automaton.states in 
  let m0 = {states=states'; univ=automaton.univ; delta=automaton.delta; sub = automaton.sub;
            gen=automaton.gen; cache_aty=cache_aty'; cache_ty=automaton.cache_ty} in
    match aty with
       ITbase(q) -> 
         let delta' = (([], Tbase(q)), x)::!(automaton.delta) in
         let m = {states=m0.states; univ=m0.univ; delta=ref delta'; sub=m0.sub;
                gen=m0.gen; cache_aty=m0.cache_aty; cache_ty=m0.cache_ty} in
           (x, m)
    | ITfun(ty, aty1) ->
        let (y, m1) = ty2automaton ty m0 in
        let (z, m2) = aty2automaton aty1 m1 in
        let delta' = (([y;z], Tfun), x)::!(m2.delta) in
        let m =  {states=m2.states; univ=m2.univ; delta=ref delta'; sub=m2.sub;
                gen=m2.gen; cache_aty=m2.cache_aty; cache_ty=m2.cache_ty} in
           (x, m)
    | _ -> raise (Fatal "aty2automaton: pair or case types")
and ty2automaton ty automaton =
  try
    let x = List.assoc ty automaton.cache_ty in
      (x, automaton)
  with Not_found ->
  let x = gen_newstate() in
  let cache_ty' = (ty,x)::automaton.cache_ty in
  let states' = x::automaton.states in
  let m0 = {states=states'; univ=automaton.univ; delta=automaton.delta; sub=automaton.sub;
            gen=automaton.gen; cache_aty=automaton.cache_aty; cache_ty=cache_ty'} in
  match ty with
     [] -> (* top *)
           raise (Fatal "ty2automaton: top type should already exist")
 (**        let delta' = (([], Ttop),x)::delta in **)
 (**        let m1 = {states=m0.states; univ= ref (union_st [x] !(m0.univ)); 
  **                    delta=m0.delta; sub=m0.sub;
  **                     gen=m0.gen; cache_aty=m0.cache_aty; cache_ty=m0.cache_ty} 
  **           in
  **             (x, m1)
  **)
  | ((ITfun(_,_)::_) | (ITbase _::_))->
      let (qs, m1) = atylist2automaton ty m0 in
      let delta1 = List.sort compare (List.map (fun q->(([q],Tepsilon),x)) qs) in
      let m2 = {states=m1.states; univ= ref (union_st [x] !(m1.univ)); 
                delta= ref (union_delta delta1 !(m1.delta)); sub=m1.sub;
  ...
  | _ -> (* pair types *)
      ...
      let delta3 = union_delta [(([x1;x2], Tpair), x)] !(m2.delta) in
      let m3 = {states=m2.states; univ=m2.univ;
                delta= ref delta3; sub=m2.sub;
                gen=m2.gen; cache_aty=m2.cache_aty; cache_ty=m2.cache_ty}  in
        (x, m3)
...
let rec subtype_aux x y m tmp =
  if x=y then (true, m)
  else if List.mem (x,y) !(m.sub) || List.mem (x,y) tmp
  then
     (true, m)
  else 
  let tmp' = (x,y)::tmp in
   if is_univstate m y then
     ...
     let tr1 = get_tr x !(m.delta) in
     let tr2 = get_tr y !(m.delta) in
...
let subtype x y m =
  if x=y || List.mem (x,y) !(m.sub) then
     (true, m)
  else
     ...
     | (_,Tepsilon)::_ ->
         if is_exstate m x then 
            ...
            let (s, referenced) = print_ity qs m traversed' in
              if !debug || List.mem x referenced then
...
let dprint_ty x m = 
  (debug := true; print_ty x m; debug := false)
...
let dprint_te mte m = 
  (debug := true; print_te mte m; debug := false)
...
let add_to_delta m delta =
    m.delta := merge_and_unify compare !(m.delta) delta
...
let tl2ty m tl =
  ...
  | _ -> 
     ...
     with Not_found ->
       let x = gen_newstate() in
       let tr = List.map (fun q1 -> (([q1],Tepsilon),x)) tl in
        (add_to_delta m tr;
         add_to_univ m [x];
  ...
  match qs with
   ...
   | _ ->
     ...
     with Not_found ->
       ...
       let new_delta = List.map (fun act -> (act, x)) tr in
        (add_to_delta m new_delta;
...
(**  let delta2 = copy_path p q3 q2 in **)
    (m.delta := delta1@(delta2@ !(m.delta));
     q3)
...
let mk_newstate tr m flag =
  ...
  with
   Not_found -> 
     let q = gen_newstate() in
     let delta1 = List.map (fun x->(x,q)) tr in
     let _ = if flag then
                 m.univ := q::!(m.univ) 
             else () in
     let _ = add_to_delta m delta1 in
       ...
       let ty1' = List.filter 
                 (fun aty1 -> 
                   List.exists (fun aty2 ->
                            (is_subtype aty1 aty2 m)) ty2)
                  ty1
       in
       let mte1'' = filter_out_suptype mte1' mte2 m in
          (f, ty1')::mte1''

let diff_most_important diffs =
  let diffs' = List.map (fun ((q1,q2),_,_,path) -> (q1,q2,path)) diffs in
  let l = list_count diffs' in
  let d = list_max (fun (x,_)->fun (y,_)-> compare x y) l in
      fst d

let generalize te cte dmap g =
  let (mte1, m1) = te2automaton te empty_automaton in
  let (mcte, m1) = te2automaton cte m1 in
  let diffs = diff_in_te mte1 m1 in
  let (q1,q2,path) = diff_most_important diffs in
  let q = merge_type m1 q1 q2 path in
  let mte2 = gen_te mte1 m1 [q] in
  let mte3 = filter_out_suptype mte2 mte1 m1 in
  let mte4 = compute_te m1 mte3 mcte mte1 dmap g in
  let mte5 = filter_out_suptype mte1 mte4 m1 in
  let mte6 = unionTE mte4 mte5 in
  let _ = print_string "generalized TE:\n" in
  let _ = print_te mte6 m1 in
    ()
