    ...
    (fun i ->
     if i < !Fpat.Global.number_of_extra_params then
       ...
       let ps =
         List.unfold
           (fun i ->
            if i < (List.length bvs' +
                      if !Fpat.Global.enable_coeff_const then 1 else 0) then
  ...
  let rec aux rfs bvs exs t =
    let desc =
      ...
      | Syntax.Fun(y, t1) ->
         ...
         let f, _ =
           List.fold_left
             (fun (f, ty) y ->
              (fun t ->
               f {Syntax.desc=Syntax.Fun(y, t); Syntax.typ=ty}),
         ...
         let tss =
           List.mapi
             (fun i t ->
              ...
              | Type.TPair(_, _)(* ToDo: fix it *) ->
                 new_params
                   (if recursive then Some(List.nth xss i) else None)
                   bvs exs
              | _ -> [])
             ts'
         in
      ...
      | Syntax.BinOp(op, t1, t2) ->
         Syntax.BinOp(op, aux rfs bvs exs t1, aux rfs bvs exs t2)
      | Syntax.Not t1 ->
         Syntax.Not (aux rfs bvs exs t1)
      | Syntax.Event(s,b) ->
         Syntax.Event(s,b)
      | Syntax.Record fields ->
         Syntax.Record
           (List.map
              (fun (f,(s,t1)) -> f,(s,aux rfs bvs exs t1))
              fields)
      | Syntax.Proj(i,s,f,t1) ->
         Syntax.Proj(i,s,f,aux rfs bvs exs t1)
      | Syntax.SetField(n,i,s,f,t1,t2) ->
         Syntax.SetField(n,i,s,f,aux rfs bvs exs t1,aux rfs bvs exs t2)
      ...
      | Syntax.TryWith(t1,t2) ->
         Syntax.TryWith(aux rfs bvs exs t1, aux rfs bvs exs t2)
      | Syntax.Pair(t1,t2) ->
         Syntax.Pair(aux rfs bvs exs t1, aux rfs bvs exs t2)
