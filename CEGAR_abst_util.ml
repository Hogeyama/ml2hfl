...
let rec add_label {env;defs;main;attr} =
  let merge = function
    | [] -> assert false
    ...
    | [f1,xs1,t11,e1,t12; f2,xs2,t21,e2,t22] ->
        CEGAR_print.prog Format.std_formatter{env=[];defs=[f1,xs1,t11,e1,t12; f2,xs2,t21,e2,t22];main="";attr=[]};assert false
  ...
  labeled, {env;defs=defs';main;attr}
...
let add_ext_funs prog =
  ...
  ignore @@ Typing.infer {env=[]; defs=defs'; main=prog.main; attr=prog.attr};
  {prog with defs=defs'}
