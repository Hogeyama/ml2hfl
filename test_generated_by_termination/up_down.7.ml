...
and down (prev_set_flag_down_139:bool) (s_prev_down_x_138:int) (x:int) =
  if prev_set_flag_down_139
  then
    if s_prev_down_x_138 > x && x >= 0 then () else assert false;
  down_without_checking_160
    prev_set_flag_down_139 s_prev_down_x_138 x
and down_without_checking_160 (_:bool) (_:int) (x:int) =
  ...
  then
    ...
    down_without_checking_160 set_flag_down_140 s_down_x_137 (x - 1)
and up (set_flag_down_140:bool) (s_down_x_137:int) (x:int) =
  if x = 0 then () else up set_flag_down_140 s_down_x_137 (x + 1)
let main (set_flag_down_140:bool) (s_down_x_137:int)
        (():unit) =
  let t1 = Random.int 0
  in
  let t2 = Random.int 0
  ...
  then
    app
      ...
      set_flag_down_140 s_down_x_137 down set_flag_down_140
      s_down_x_137 t1
  else
    (if t2 < 0
     then
