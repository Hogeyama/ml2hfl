...
(* $Id: semant.ml,v 1.3 2011-08-08 19:31:17 weis Exp $ *)
...
let init_semantics caml_fun =
    Funval
      (function
       | Numval n ->
           Funval(function
                  | Numval m -> Numval(caml_fun n m)
                  | Funval _ -> raise Illtyped)
       | Funval _ -> raise Illtyped);;
...
let rec semant rho =
  let rec sem = function
    ...
    | App(e1,e2) -> (match sem e1 with
                     | Funval f -> f (sem e2)
                     | Numval _ -> raise Illtyped) in
  sem
;;

let semant_asl (Decl(_s,e)) = semant !global_sem e
;;

let print_semval = function
| Numval n -> print_string "Numval "; print_int n
| Funval _f -> print_string "Funval <fun>"
;;
