...
(* $Id: typing.ml,v 1.4 2011-08-08 19:31:17 weis Exp $ *)
...
let rec unify (tau1, tau2) =
  ...
  | (Unknown | Number | Arrow (_, _) | TypeVar _ as t1),
    (TypeVar _ | Arrow (_, _) | Number | Unknown as t2)
...
let rec asl_typing_expr gamma =
  ...
  | Cond (e1, e2, e3) ->
      ...
      in unify(Number, type_rec e1); unify(t2, t3); t3
  | App(Abs(_x, e2), e1) -> (* LET case *)
...
let print_type_scheme (Forall(gv, t)) =
 ...
 let names =
   ...
   | (_n, []) -> []
   | (n, (_v1 :: lv)) -> tvar_name n :: names_of (n + 1, lv) in
   names_of (1, gv) in
