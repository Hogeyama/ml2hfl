...
(* $Id: lexuniv.ml,v 1.5 2011-08-08 19:31:17 weis Exp $ *)
...
let rec lire_commentaire = parser
  | [< ''\n' >] -> ()
  | [< '_c; flux >] -> lire_commentaire flux
;;

let mc_ou_ident table_des_mots_clés ident =
    try Hashtbl.find table_des_mots_clés ident with
...
let mc_ou_erreur table_des_mots_clés caractère =
    ...
    try Hashtbl.find table_des_mots_clés ident with
    | Not_found -> raise (Stream.Error ("Illegal character " ^ ident))
;;
