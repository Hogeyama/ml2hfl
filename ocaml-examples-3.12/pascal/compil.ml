...
(* $Id: compil.ml,v 1.3 2011-08-08 19:31:17 weis Exp $ *)

open Syntaxe;;
open Envir;;
open Printf;;

(* Un mot = quatre octets *)
let taille_du_mot = 4;;
...
let rec type_de env = function
  | Constante (Entière _) -> Integer
  | Constante (Booléenne _) -> Boolean
  | Variable nom -> (cherche_variable nom env).typ
  | Application (fonc, _) ->
      (cherche_fonction fonc env).fonc_type_résultat
  | Op_unaire (op, _arg) ->
      let (_type_arg, type_res) =
        Typage.type_op_unaire op in
      type_res
  | Op_binaire (op, _arg1, _arg2) ->
      let (_type_arg1, _type_arg2, type_res) =
        Typage.type_op_binaire op in
      type_res
  | Accès_tableau (arg1, _arg2) ->
...
let rec sans_interférences env = function
  ...
  | Application (_fonc, _args) -> false
  | Op_unaire (_op, arg) ->
      sans_interférences env arg
  | Op_binaire (_op, arg1, arg2) ->
...
let rec besoins env = function
  | Constante _
  | Variable _ -> 0
  | Application (_fonc, _args) -> dernier_registre
  | Op_unaire (_op, arg) -> besoins env arg
  | Op_binaire (_op, arg1, arg2) -> besoins_op_binaire env arg1 arg2
...
and affecte_var env nom reg =
  ...
  | Local_indirect n ->
      printf "store sp, %d, r %d  # %s \n"
             (!profondeur_pile - n) reg nom
  | Global_direct _ | Local_direct _ ->
      failwith "mauvaise gestion des emplacements de variables"
;;
