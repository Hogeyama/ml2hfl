...
(* $Id: txthanoi_fra.ml,v 1.5 2011-08-08 19:31:17 weis Exp $ *)

(* Le jeu des Tour de Hanoi animé en mode texte. *)
...
let imprime_jeu nombre_de_disques départ milieu arrivée =
    ...
    let b = base_de_tige nombre_de_disques in
    imprime (b ^ b ^ b)
;;

let ajoute_disque disque (décalage, disques) =
    (décalage - 1, disque :: disques)
;;

let sommet = function
  | (_décalage, sommet :: _reste) -> sommet
  | (_décalage, []) -> failwith "sommet: tige vide"
;;

let enlève_sommet = function
  | (décalage, _sommet :: reste) -> (décalage + 1, reste)
  | (_décalage, []) -> failwith "enlève_sommet: tige vide"
;;
