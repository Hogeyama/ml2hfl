...
(* $Id: grahanoi_fra.ml,v 1.6 2011-08-11 15:57:09 weis Exp $ *)

(* Le jeu des Tour de Hanoi animé en mode graphique. *)
...
let text_size_x s =
  let x, _ = text_size s in
  x
;;

let text_size_y s =
  let _, y = text_size s in
  y
;;
...
let déplace (_nom_départ, départ) (_nom_destination, destination) =
...
let jeu nombre_de_disques =
  if nombre_de_disques < 0 then failwith "Un nombre positif de disques SVP!";
  clear_graph ();
...
let usage () =
  prerr_endline "Usage: hanoi <nombre positif de disques>";
  exit 2
;;
...
if !Sys.interactive then () else begin
  ...
  if l <= 1 then usage ();
  let nombre_de_disques =
    try int_of_string (Sys.argv.(1)) with
    | Failure "int_of_string" -> usage () in
  jeu nombre_de_disques;
  attend ();
  exit 0
end
;;
