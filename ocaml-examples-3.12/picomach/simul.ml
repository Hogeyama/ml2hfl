...
(* $Id: simul.ml,v 1.4 2011-08-08 19:31:17 weis Exp $ *)
...
let lire_instruction adresse =
  ...
  if adr < 0 || adr >= Array.length pico.code then
...
let lire_mémoire adresse =
  let adr = adresse/taille_du_mot in
  if adr < 0 || adr >= Array.length pico.mémoire then
...
let exécute_appel_système appel argument =
  if appel < 0 || appel >= Array.length tableau_des_appels_système
