...
(*
      withtype {i0:nat}{i1:int}{i2:int}{i3:int | i0 <= i1 <= i2 <= i3}{id:nat}
               int(i0) -> int(i1) -> int(i2) -> int(i3) -> int(id) -> unit
*)
      in
      let rec loop2 is id =
...
(*
      withtype {is:nat}{id:nat | id >= n2} int(is) -> int(id) -> unit
*)
      in
      loop2 j (2 * n2)
    done; loop (n2 / 2) (n4 / 2)
(*
    withtype int(n2) -> int -> unit
*)
  in
    loop n (n / 4);
...
(*
    withtype {i0:nat}{i1:int | i0 <= i1} int(i0) -> int(i1) -> {id:nat} int(id) -> unit
*)
    in
    let rec loop2 is id =
      if is >= n then () else begin
...
(*
    withtype {is:nat}{id:nat | id >= 4} int(is) -> int(id) -> unit
*)
    in
    loop2 1 4;
...
(*
    withtype {j:nat}{k:nat | k <= n / 2} int(j) -> int(k) -> [i:nat | i <= n] int(i)
*)
    in
    let rec loop2 i j =
...
(*
    withtype {i:nat} int(i) -> {j:nat | j <= n} int(j) -> unit
*)in
    loop2 1 1; n
(*
...
(*
  withtype {i:nat} int(i)  -> float -> float -> int -> int -> float * float
*)
  in
