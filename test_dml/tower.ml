(* {size:int | size > 0} *)
let play size =
  let leftPost = Array.make size 0
  and middlePost = Array.make size 0
  and rightPost = Array.make size 0 in
...
(*
  withtype {n:nat}{s:nat}{p:nat}{p':nat |
            ...
            s + n <= size /\ n <= p /\ n <= p' }
           int(n) * int vect(size) * int(s) * int vect(size) * int(p) * int vect(size) * int(p') -> unit
*)
  in
  showposts();
