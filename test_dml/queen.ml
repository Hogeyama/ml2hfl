...
(*
  withtype {i:nat | i < size} int(i) -> int -> unit
*)
  in
...
(*
    withtype {i:nat | i <= size } int(i) -> unit
*)
    in
    aux(0); print_string "\n" in
  let test j =
...
(*
    withtype {i:nat | i < size} int(i) -> bool
*)
    in aux 0
(*
  withtype {j:nat | j < size} int(j) -> bool
*)
  in
  let rec loop row =
...
(*
  withtype {row:nat | row < size} int(row) -> unit
*)
  in loop(0)
(*
