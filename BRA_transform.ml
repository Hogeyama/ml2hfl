...
let restore_ids =
  ...
  let sub = function
    | {desc = Let (rec_flag, bindings, cont); typ = t} ->
      {desc = Let (rec_flag, List.map (fun (f, args, body) -> (trans_id f, List.map trans_id args, body)) bindings, cont); typ = t; attr=ANone}
    | {desc = Fun (f, body); typ = t} -> {desc = Fun (trans_id f, body); typ = t; attr=ANone}
    | {desc = Var v; typ = t} -> {desc = Var (trans_id v); typ = t; attr=ANone}
    | t -> t
...
let randomized_application f t =
  let rec aux f args = function
    | t when is_base_typ t -> {desc = App (f, args); typ = t; attr=ANone}
    | TFun ({Id.typ = t1}, t2) ->
      let r =
	match t1 with
	  | TUnit -> unit_term
...
let rec regularization e =
  match find_main_function e with
    | Some ({Id.name = "main"} as f) ->
      let main_expr = randomized_application {desc = Var f; typ = Id.typ f; attr=ANone} (Id.typ f) in
      let rec aux = function
  ...
  let hole_insert target state typed =
	...
	if id = target.id then
	  ...
	  let app_assert =
	    make_let
	      [Id.new_var ~name:"_" TUnit, [], make_if prev_set_flag (make_if hole_term unit_term (make_app fail_term [unit_term])) unit_term]
	      {desc = App (make_var id', List.map make_var args'); typ = Id.typ id'; attr=ANone}
	  in
