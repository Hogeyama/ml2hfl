...
let rec check t typ =
  ...
  | {desc=Event _; typ=TFun(x,TUnit)} ->
      check_var x TUnit
  | {desc=Record _; typ=_} -> assert false
  | {desc=Proj _; typ=_} -> assert false
  | {desc=SetField _; typ=_} -> assert false
  | {desc=Nil; typ=_} -> assert false
  | {desc=Cons _; typ=_} -> assert false
  | {desc=Match _; typ=_} -> assert false
  | {desc=TryWith _; typ=_} -> assert false
  | _ ->(*
      match t.desc with
        Unit -> assert false
      | True -> assert false
