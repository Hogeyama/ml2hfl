...
let rec verify_aux g m steps trials redexes1 freezed dmap cte nte counter =
  if trials =0 then
     ...
     let _ = debug ("# of reduction so far: "
                    ^(string_of_int !(Reduce.redcount))^"\n") in
     let _ = show_time() in
     ...
     ***)
     let _ = Reduce.set_flags true redexes1' freezed in
...
**)
     let telist' = telist in
     let _ = if !debugging then 
                (print_string "Candidates:\n";
                 print_te telist'; flush stdout)
             else () in
     let te = list2hash telist' in
     let _ = if !expand then expand_te te cte nte (sym_dmap dmap) g else () in
     let telist'' = hash2list te in
     let _ = if !debugging then 
                (print_string "Expanded Candidates:\n";
                 print_te (diff_telist telist'' telist'); flush stdout)
             else () in
     (*** for debugging 
     let _ = debug "***********\n" in
     ...
     ***)
     ...
     let te' = compute_te te cte nte !(Grammar.dmaprec) g in
     let new_telist = hash2list te' in
     let _ = (te_updated:= not(new_telist=[])) in
     let nte' = add_te nte new_telist in
(**
     let _ = print_string "current types\n" in
     let _ = 
       print_te (List.filter (fun (f,_)-> is_recfun f) (hash2list nte')) in
     let _ = Propagate.sanity_check cte nte' !(Grammar.dmaprec) g false in
**)
     let _ = if !propagation then 
               (Propagate.propagate_te new_telist cte nte' g;
(*               print_te (hash2list nte'); *)
               Propagate.sanity_check cte nte' dmap g true )
     in
     let _ = show_time() in
     let _ = debug "type check ended\n" in
     let _ = debug "Inferred type\n" in
     let _ = if !debugging then (print_te new_telist; flush stdout) else () in 
 ...
 **)
     ...
     let _ = Reduce.set_flags false redexes1' freezed in
...
let verify g m nte =
  ...
  let _ = Grammar.mk_dmaprec dmap in
...
let rec verifyParseResult (prerules,tr) = 
  ...
  | LimitReached te -> 
     (if !debugging then
      ...
      if !rep>1 then 
        ...
        Reduce.reset_hashtab();
        rep := !rep-1;
        repeat prerules tr g m (list2hash te))
      else print_string "Verification failed (time out).\n")
  ...
  with
    Grammar.DuplicatedNonterminal f -> (print_string ("Syntax Error: More than one rule for "^f^"\n"))
    | Grammar.DuplicatedVars(f,v) -> (print_string ("Syntax Error: Duplicated variable "^v^" in "^f^"\n"))

and repeat prerules tr g m nte = 
    let (te,cte,dmap) = verify g m nte in
    let _ = print_string "The property is satisfied.\n" in
    let _ = if !output="" then ()
            else let fp = open_out !output in
...
let rec read_options index =
  ...
  | "-exp" -> (expand := true; read_options (index+1))
  | "-prop" -> (propagation := true; read_options (index+1))
  | "-o" -> (output := Sys.argv.(index+1); read_options (index+2))
  | "-g" -> (gen := int_of_string(Sys.argv.(index+1));
             read_options (index+2))
  | "-depth" -> (Reduce.depth := int_of_string(Sys.argv.(index+1));
             read_options (index+2))
  ...
  | "-bnp" -> (Reduce.depthfirst := false; factor := 2; 
               Reduce.pruning := false;Reduce.pruning_eager := false; 
	       read_options(index+1))
  | "-np" -> (Reduce.pruning := false; Reduce.pruning_eager := false; 
              read_options(index+1))
  | "-pr" -> (Reduce.pruning_count := int_of_string(Sys.argv.(index+1));
              read_options(index+2))
  | "-m" -> (Reduce.mixed := true; read_options(index+1))
  | "-d2" -> (Reduce.depth2 := true; read_options(index+1))
  | "-nrv" -> (Reduce.flag_reset_visited := false; read_options(index+1))
...
let main () =
  let _ = print_string "TRecS 1.30: Type-based model checker for higher-order recursion schemes\n" in
