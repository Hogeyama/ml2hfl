...
let flag_reset_visited = ref true
let depthfirst = ref true
let depth2 = ref false
let breadth2 = ref false
let pruning = ref true
let width = ref 3
let pruning_eager = ref true 
let pruning_count = ref 118
let mixed = ref false
let safemode = ref true
...
type flag = bool
**)
type flag = G of bool | Y of bool (* G means the flag has been completely set. Y means that the flag is being set. *)
...
let rec string_of_trace trace = string_of_trace_aux  (List.rev_append trace [])
and string_of_trace_aux trace = 
  match trace with
    [] -> ".\n"
  | (n,d)::trace' ->
       ("("^n^","^(string_of_int(d))^")"^(string_of_trace_aux trace'))

let print_trace trace = print_string (string_of_trace trace)
...
let reset_visited (vl1,vl2) = (vl1,[]) 
**)
type lastvisit = string
type visited = lastvisit * ((nameNT * state ) * int) list
let empty_visited = ("", [])

let rec add_to_visited (f,q,i) visited =
  let (last, vl1) = visited in 
  let last' = f^"_"^(string_of_int i) in
  let f' = f^"@"^last in  
  let f0 = String.sub f 0 (String.index f '#') in
    if Hashtbl.mem Grammar.recfuntab f0 then
(*       print_string ("registering "^f0^"\n"); *)
       (last', add_to_visited_aux (f',q) vl1 [])
    else (last', vl1)
and add_to_visited_aux (f,q) visited newv =
  match visited with
     [] -> ((f,q),1)::newv
   | ((f',q'),j)::visited' ->
         if f=f' && q=q' then 
            ((f,q),j+1)::(List.rev_append visited' newv)
         else
            add_to_visited_aux (f,q) visited' (((f',q'),j)::newv)

let visited_twice (n,q) visited =
  let (last,vl) = visited in
  let n' = n^"@"^last in
  let b =
     try
      List.assoc (n',q) vl >= !depth
     with
      Not_found -> false
  in
(*  let _ = if b then debug (n'^" is visited twice\n") in *)
    b

(* dummy *)
let reset_visited x = x

(**
let rec print_visited (_,visited2) =
  (print_string "visited:";
   List.iter (fun (f,q,i) -> print_string (f^(string_of_int i)^" ")) visited2;
   print_string "\n")
**)
let rec print_visited (last,vl) =
  (print_string "visited:";
   List.iter (fun ((f,q),i) -> 
		  print_string (f^":"^q^":"^(string_of_int i))) vl;
   print_string "\n")
...
let rec reset_visited_info_in_redex redex =
  let (eterm,ety,trace)=redex in
    (reset_visited_info_in_eterm eterm, ety, trace)
and reset_visited_info_in_eterm eterm =
  match eterm with
    EheadNT(f,tinfo,visited,id) -> EheadNT(f,tinfo,reset_visited visited,id)
  | Eapp(eterm1,eterm2,tinfo,id) -> Eapp(reset_visited_info_in_eterm eterm1, eterm2,tinfo,id)
  | _ -> eterm
...
let empty_tinfo:tinfo = ref ([],G(false))
let new_tinfo() =
  ref ([], G(false))
...
let is_processed eterm ety = 
  ...
  try
    let (eterm1,b) = Hashtbl.find processed (id, ety) in
      Some(eterm1,b)
  with
   Not_found -> 
       (Hashtbl.add processed (id,ety) (eterm, false);
        None)
let update_processedtab eterm ety =
  let id = id_of_eterm eterm in
    Hashtbl.replace processed (id, ety) (eterm, true)

let reset_hashtab() =
  (Hashtbl.clear processed;
   Hashtbl.clear tidtab;
   Hashtbl.clear ntidtab;
   Hashtbl.clear appidtab;
   Hashtbl.clear constEtermTab;
   Hashtbl.clear Grammar.recfuntab
   )
...
let initial_redex g q0 =
  ...
  let tinfo = ref ([ETbase q0],G(false)) in
(**  let s = g.s in **)
...
let rec fdlist2string fdlist =
  match fdlist with
   ...
   | Some(n)::fdlist' ->
        "#"^(string_of_int n)^(fdlist2string fdlist')
...
**)
 let visited' = (eterm1,eterm2)::visited in
  match (eterm1, eterm2) with
    (EheadNT(f,tinfo1,visited1,_), EheadNT(_,tinfo2,visited2,_))
        -> (mk_link_tinfo tinfo1 tinfo2; 
            debug ("linking "^f^":\n");
            if !debugging then 
             (print_visited visited1;
...
let mk_link eterm1 eterm2 =
   let _ = debug ("creating a link on a term of id: "^(string_of_int(id_of_eterm eterm1))^"\n") in
(*   let _ = if !debugging && id_of_eterm eterm1 < 2000 then print_eterm eterm1 in *)
   let _ = debug "\n" in
(**
                  let _ = debug "\nto:\n" in
                  let _ = print_eterm eterm1 in
         ...
         ( match processed with
             Some(eterm1, b) ->
               if b then 
                ...
                (* eterm1 has not been processed yet *)
                (* to do: the direction of the link should depend on
                   the scheduling. *)
                  let _ = mk_link eterm1 eterm'; mk_link eterm' eterm1 in 
                  (* Should eterm1 in the hash table be replaced by eterm'? *)
                    (eterm', ety, tr)::redexes
          | None -> let _ =  process_head eterm' ety in
                (eterm', ety, tr)::redexes
         )
...
(**  let _ = if !debugging && !redcount mod 1000 = 0 then
            (print_string ((string_of_int !redcount)^"th reduction\n");
  ...
  in
**)
  match is_processed eterm ety with
   Some(_,true) -> ([], false)
 | _ ->
  let _ = (redcount := !redcount+1) in
  let _ = update_processedtab eterm ety in
          ...
          ( match processed with
             Some(eterm1, b) ->
               if b then
                ...
                let _ = mk_link eterm1 eterm2; mk_link eterm2 eterm1 in 
...
**)
            ...
            **)
               let redex' = if !flag_reset_visited then reset_visited_info_in_redex redex else redex in
                 divide_redexes_aux eterms' visited red1 ((redex', [])::red2))
             else
  ...
  match q with
   ...
   | ((eterm,_),_)::q' ->
       ( print_eterm eterm;
        ...
        print_string (q^"\n");
        let f = head_of_eterm eterm in
        let _ = match f with
            EheadNT(_,_,visited,_) -> 
...
let print_freezed freezed = ()
(*
    print_queue (List.map (fun ((eterm,_,_),_) -> ((eterm,0),0)) freezed)
*)
...
(**
merge_and_unify compare [aty] ty
**)
...
let rec remove_opentype ty checked =
  ...
  | (aty1,flag1)::ty' ->
      ...
      let _ = if !safemode && List.length tylist >3000 then
               ((if !debugging then print_string "completeness given up\n" else ());
                complete := false)
(*
              let tylist' = List.filter (fun ty -> List.length ty < !width && Typing.width_of_ty ty < !width ) tylist in 
               if List.length tylist'>3000 then
                  raise GiveUp
               else   
                 (print_string "too many candidates: gave up completeness\n"; tylist')
              else tylist
*)
      in
      if (** true || **)
...
let lookup_tinfo tinfo =
  match !tinfo with
     (etys, G(b)) -> (etys, b)
   | (etys, Y(b)) -> assert false (* flag has not been set completely *)
  ...
  with
    Not_found -> 
      if List.memq tinfo visited then ([], false, [tinfo])
        ...
        let (etys, flag) = lookup_tinfo tinfo in 
...
(*** ??????????????? cycles'=[] should be replaced by visited=[]? ***)
        let _ = if cycles'=[] then (tinfo := (new_etys, G(flag))) else () in 
...
let set_flag_tinfoG tinfo =
  match !tinfo with
     (_, G(_)) -> ()
   | (ty, Y(b)) ->  tinfo := (ty,G(b))

let flag_of_tinfo tinfo =
  let (_,flag) = !tinfo in flag

let rec set_flag_in_etermY b eterm =
  match eterm with
   EheadNT (_,tinfo,_,_) -> set_flag_tinfo (Y(b)) tinfo
 | EheadT (_,tinfo,_) -> set_flag_tinfo (Y(b)) tinfo
 | EheadC (_,tinfo,_) -> set_flag_tinfo (Y(b)) tinfo
 | EheadFD (_,tinfo,_) -> set_flag_tinfo (Y(b)) tinfo
 | Epair (tinfo,_) -> set_flag_tinfo (Y(b)) tinfo
 | Edpair (tinfo,_) -> set_flag_tinfo (Y(b)) tinfo
 | Eapp(e1,e2,tinfo,_) ->
     if flag_of_tinfo tinfo=Y(b) then
        ()  (* flag has been already set *)
     else
      (set_flag_tinfo (Y(b)) tinfo;
       set_flag_in_etermY b e1; 
       set_flag_in_etermY b e2)

let rec set_flag_in_etermG eterm =
  match eterm with
   EheadNT (_,tinfo,_,_) -> set_flag_tinfoG tinfo
 | EheadT (_,tinfo,_) -> set_flag_tinfoG tinfo
 | EheadC (_,tinfo,_) -> set_flag_tinfoG tinfo
 | EheadFD (_,tinfo,_) -> set_flag_tinfoG tinfo
 | Epair (tinfo,_) -> set_flag_tinfoG tinfo
 | Edpair (tinfo,_) -> set_flag_tinfoG tinfo
 | Eapp(e1,e2,tinfo,_) ->
     match flag_of_tinfo tinfo with
       G(_) -> ()   (* flag has been set to green already *)
     | _ ->
      (set_flag_tinfoG tinfo;
       set_flag_in_etermG e1; 
       set_flag_in_etermG e2)

let rec set_flag_in_redexY b eterm =
  set_flag_in_etermY b eterm
let rec set_flag_in_redexG eterm =
  set_flag_in_etermG eterm
(*
  match eterm with
   EheadNT _ -> ()
 | EheadT _ -> ()
...
*)
let set_flag_squeueY b q =
   List.iter (fun (((eterm,_,_),_),_) -> set_flag_in_redexY b eterm) q

let set_flag_squeueG q =
   List.iter (fun (((eterm,_,_),_),_) -> set_flag_in_redexG eterm) q

let set_flag_queueY b (q1,q2) =
  (List.iter (fun sq -> set_flag_squeueY b sq) q1;
   List.iter (fun  sq -> set_flag_squeueY b sq) q2)

let set_flag_queueG (q1,q2) =
  (List.iter (fun sq -> set_flag_squeueG sq) q1;
   List.iter (fun  sq -> set_flag_squeueG sq) q2)

(**
let set_flag_queue b (q1,q2) =
   set_flag_queueY b (q1,q2); set_flag_queueG (q1,q2)
**)
let set_flag_freezedY b redexes =
  List.iter (fun ((eterm,_,_),_) -> set_flag_in_redexY b eterm) redexes
let set_flag_freezedG redexes =
  List.iter (fun ((eterm,_,_),_) -> set_flag_in_redexG eterm) redexes

let set_flags b queue freezed =
  (set_flag_queueY b queue;
   set_flag_freezedY b freezed;
   set_flag_queueG queue;
   set_flag_freezedG freezed)
...
let rec elim_eterm eterm td =
  let aty = aty_of_td td in
  let tinfo = get_tinfo eterm in
    ((if !pruning_count=1 && !debugging then
     ( print_string ("eliminating id:"^(string_of_int(id_of_eterm eterm))^" with type ");
     print_aty aty; print_string "\n" ));
...
let rec dequeue_redex (q1,q2) te cte =
  ...
  | ((x, p)::qtop)::q' -> 
     let (eterm,ety,trace) = fst x in
       if !pruning_eager && !pruning_count>0 then
        (
...
(*                     (elim_eterm_top eterm td; *)
                     (elim_eterm eterm td;
                      (*** elim_eterm_top eterm td; ***)
                      (if !debugging then 
                      (print_string ("redex of id: "
                         ^(string_of_int (id_of_eterm eterm)));
                      print_string " has been eliminated\n"
                     (** if !pruning_count=1 then print_eterm eterm **)));
...
let rec red_nsteps n redexes1 freezed  g m te cte =
  if n=0 then 
      ...
      let _ = debug ("length of freezed:"^(string_of_int(List.length freezed))^"\n") in
 ...
 (**  let _ = print_queues redexes1 freezed in **)
    ...
    match v with
       None ->
        ...
        let _ = if !debugging then print_freezed freezed in
        let _ = debug ("length of freezed:"^(string_of_int(List.length freezed))^"\n") in
